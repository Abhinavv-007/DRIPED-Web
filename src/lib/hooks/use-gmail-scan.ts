...
// Cheap pre-filter for AI fallback: an email body without any currency-ish
// signal almost certainly isn't a receipt the AI can rescue, so we save a
// 1–3 s Worker round trip per email. Tuned conservatively (false-negatives
// preferred over false-positives so we never quietly drop a real receipt).
const MONETARY_HINT_REGEX = /[₹$€£¥]\s?\d|\d[\s,.]\d{2}|\b(usd|inr|eur|gbp|aed|sgd|jpy|aud)\b|\b(amount|total|charged|invoice|receipt|subscription|renewal|paid|payment)\b/i;
...
export function useGmailScan() {
  ...
  const startScan = useCallback(async () => {
    ...
    try {
      ...
      let scanned = 0;
      // 12 keeps us comfortably under Gmail's ~250 quota-units/sec/user
      // (messages.get costs 5 units → 60/s sustained at this batch size with
      // typical ~200 ms server response). Doubling from 6 cuts wall-time of
      // a 600-email scan from ~5 min to ~2 min for first-time users without
      ...
      for (let i = 0; i < messageIds.length; i += batchSize) {
        ...
        for (const result of results) {
          ...
          //    Three short-circuits make this dramatically faster without
          //    sacrificing correctness:
          //      a) Already past the threshold → trust the local parser.
          //      b) Receipt class is a refund/failed/cancellation → not a sub.
          //      c) Body has no currency or receipt-y keyword anywhere → AI
          //         can't pull a number out of nothing, so don't waste a call.
          const merchantSource = parsed.merchant?.source;
          const hasHighTrustMerchant =
            merchantSource === "domain" || merchantSource === "header";
          const monetaryHintFound =
            MONETARY_HINT_REGEX.test(subject) || MONETARY_HINT_REGEX.test(body);
          const shouldAskAi =
            parsed.overallConfidence < FALLBACK_CONFIDENCE_THRESHOLD &&
            parsed.receiptType !== "refund" &&
            parsed.receiptType !== "failed" &&
            parsed.receiptType !== "canceled" &&
            // Domain-matched merchants are already 90+ confidence on the
            // merchant axis; if we got here it's because amount/cycle were
            // unsure, and AI rarely improves those without a monetary hint.
            (!hasHighTrustMerchant || monetaryHintFound) &&
            monetaryHintFound;
          if (shouldAskAi) {
