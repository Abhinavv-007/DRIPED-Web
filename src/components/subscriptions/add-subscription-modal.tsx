...
export function AddSubscriptionModal({ open, onOpenChange, onSuccess }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    service_slug: "",
    amount: "",
    // Default the form's currency to the user's preferred currency, but let
    // them override per-subscription. Without this we silently coerced every
    // manually-added sub to whatever the global preference was, which surprised
    // users typing rupees while the app default sat at USD.
    currency: currency,
  ...
  const selectService = (svc: ServiceInfo) => {
    setForm({
      ...
      // The seed knows what currency this service is typically priced in
      // (Spotify ₹119, Netflix ₹199, ChatGPT $20, …). Honour it as a hint
      // but the user can still flip the dropdown below.
      currency: svc.defaultCurrency || currency,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const body: SubscriptionCreateBody = {
      ...
      // see the amount silently saved as USD.
  ...
  const resetAll = () => {
    ...
    setSearch("");
    setCatFilter("all");
    setForm({ service_name: "", service_slug: "", amount: "", currency: currency, billing_cycle: "monthly", status: "active", start_date: "", next_renewal_date: "" });
  };
            ...
            <motion.div key="form" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-[1fr_auto_1fr] gap-2">
                  ...
                  <div className="space-y-1.5">
                    <Label className="font-bold text-xs uppercase tracking-wider">Curr.</Label>
                    <select
                      value={form.currency}
                      onChange={(e) => update("currency", e.target.value)}
                      className="brutal-input min-w-[5.25rem] pr-2 font-mono"
