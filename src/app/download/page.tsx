...
const APK_REPO = "Abhinavv-007/DRIPED-Android";
const APK_PATH = `https://github.com/${APK_REPO}/releases/download/${APK_VERSION}/driped-${APK_VERSION.slice(1)}.apk`;
const APK_FILENAME = `driped-${APK_VERSION.slice(1)}.apk`;
const RELEASES_URL = `https://github.com/${APK_REPO}/releases`;
...
/**
 * Release timeline shown on the download page. Public releases live on
 * the DRIPED-Android GitHub repo (`Abhinavv-007/DRIPED-Android`); earlier
 * entries document internal alpha / beta milestones during 2024–2026.
 * Kept honest in the channel labels.
 */
const RELEASE_TIMELINE: ReleaseEntry[] = [
  ...
  },
  {
    ...
    date: "Apr 2026",
    channel: "public",
    title: "First public release — cloud AI rollout",
    notes: [
      "Replaced 270 MB on-device LiteRT model with Llama 3.1 8B Worker fallback.",
      "APK now ~67 MB instead of ~340 MB.",
  ...
  },
  {
    ...
    title: "Real branding",
    notes: [
      "New Driped brand mark — gold drop with ink slot, brutalist 4 px shadow.",
      "Adaptive launcher icon (Android 8+) with cream background + gold drop.",
      "White silhouette as the system notification icon (no more square).",
      "Web favicon, apple-touch icon and OG card now use the same mark.",
    ],
  },
  {
    version: "v3.2.0",
    date: "Apr 2026",
    channel: "public",
    title: "Bug fixes + faster scan",
    notes: [
      "Add-subscription form now has its own currency selector — no more silent USD coercion when typing rupees.",
      "Gmail scan ~2-3× faster: bigger fetch batches + skip AI fallback when the email has no monetary signal.",
      "Brand-logo resolver: short-form slugs (`discord`, `youtube`, `github`, …) now find their canonical asset.",
      "Tighter motion: capped list-stagger so 60+ subscriptions don't ripple in for two seconds.",
      "Home greeting wraps to two lines on narrow phones instead of clipping the name.",
    ],
  },
];
      ...
      <main className="mx-auto max-w-5xl space-y-12 px-4 py-10 md:px-8 md:py-16">
        ...
        >
          ...
          <p className="text-sm font-semibold text-foreground/70">
            ...
            Releases. <strong>v3.1.2</strong> followed with the real Driped
            brand mark, and <strong>v3.2.0</strong> ships the first wave of
            bug fixes from real-world use. Older builds are documented here
